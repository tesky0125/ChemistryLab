...
public class Fragment_Headquaters extends Fragment implements IInFragmentCallback {
	...
	private FragHeadquatersListViewAdapter myListViewAdapter;
	private HeadquatersListService expertsListService;
	...
	private Headquaters experts=null;
	private HeadquatersDao headquatersDao=null;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		expertsListService=new HeadquatersListService(mActivity);
		headquatersDao = new HeadquatersDao(mActivity);
		...
		// 使用自定义的Adapter
        myListViewAdapter = new FragHeadquatersListViewAdapter(mActivity, listdata,
...
//System.out.println(_id);
				experts = headquatersDao.getBeanByID(_id);
...
//    	mAbHttpQueue.downloadBeforeClean(itemRefresh);
	...
	private void initSendSmsListener() {
		...
		OnClickListener listener = new OnClickListener() {
			...
			public void onClick(View v) {
				...
				case R.id.btn_send:
					List<String> list=getCheckedPhones();
					if(list.size()>0)
						mInActivityListener.onSendSms(list);
					else
						mActivity.showToast("请选择联系人！");
					break;
	...
	private List<String> getCheckedPhones()
	{
		...
		for(Map<String,Object> map : listdata)
		{
			...
			Integer _id=(Integer) map.get("_id");
			Headquaters experts = headquatersDao.getBeanByID(_id);
			if(isCheck) 
		...
		itemRefresh.callback = new AbHttpCallback() {
			...
			public void get() {
	   		    try {
	   		    	...
	   		    	newListdata=expertsListService.getHeadquatersList(searchWhereType, currentPageNum, Constant.pageSize, searchWhereClause);
//System.out.println("headquatersListService--newListdata"+newListdata.size());		   		    	
	   		    	totalCount=expertsListService.getHeadquatersCount(searchWhereType, searchWhereClause);
		...
		itemLoadMore.callback = new AbHttpCallback() {
			...
			public void get() {
	   		    try {
	   		    	...
	   		    	List<Map<String, Object>> loadMoreList=expertsListService.getHeadquatersList(searchWhereType, currentPageNum, Constant.pageSize, searchWhereClause);
